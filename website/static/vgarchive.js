$(document).ready(function () {
  ...
  function updateGame(game_id, completed_response, recommend_response) {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        // Update the game's completed and recommend status in the database
        if (
          completed_response !== undefined ||
          recommend_response !== undefined
        ) {
          // Prepare the data object
          var data = {};
          if (completed_response !== undefined) {
            game.completed = completed_response;
            data.completed = game.completed ? "true" : "false";
          }
          if (recommend_response !== undefined) {
            game.recommend = recommend_response;
            data.recommend =
              recommend_response === null ? null : recommend_response;
          }

          // Send a POST request to update the game in the database
          $.ajax({
            ...
            success: function (response) {
              ...
              // Update the game status in the table
              if (response.success) {
                var row = $("button[data-id='" + game_id + "']").closest("tr");
                if (completed_response !== undefined) {
                  row
                    .find(".completed")
                    .text(completed_response ? "Yes" : "No");
                }
                if (recommend_response !== undefined) {
                  var recommendCell = row.find(".recommended");
                  if (recommend_response === null) {
                    recommendCell.text("Unrated");
                  } else if (recommend_response) {
                    recommendCell.text("Like");
                  } else {
                    recommendCell.text("Dislike");
                  }
                }
              } else {
                console.log("Error updating game:", response.error);
      ...
      success: function (response) {
        ...
        for (var i = 0; i < gameCollection.length; i++) {
          ...
          if (game.platform && game.platform.toLowerCase() === "steam") {
            $("<button>")
              .addClass("delete disabled") // Add the "disabled" class here
              .attr("data-id", game.id)
  ...
  $(document).on("click", ".edit", function () {
    ...
    var completed =
      $(this).closest("tr").find(".completed").text().trim().toLowerCase() ===
      "yes";
    ...
    showDialogBox(completed_msg, function (response1) {
      ...
      if (response1.toLowerCase() === "cancel") {
        return;
      }
      ...
      showDialogBox(recommend_msg, function (response2) {
        ...
        if (response2.toLowerCase() === "cancel") {
          return;
        }
